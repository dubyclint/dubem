...
def format_prediction_output(data):
    match = html.escape(f"{data['match']}")
    winner = html.escape(f"{data['winner']}")
    pattern_summary = html.escape(f"{data['pattern_summary']}")

    return (
        f"<b>🔮 Prediction for {match} ({data['date']})</b>\n"
        f"<b>🏆 Winner:</b> {winner} (<i>{data['win_percent']*100:.0f}%</i>)\n"
        ...
        f"<b>🔁 Reoccurring Patterns:</b> {pattern_summary}"
    )
