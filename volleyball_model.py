...
import numpy as np
from h2h_collector import search_volleyball_team, get_last_six_volleyball_matches
from pattern_detector import detect_volleyball_pattern

WEIGHTS = {
    "attack_efficiency": 0.3,
    "block_rating": 0.2,
    "receiving_quality": 0.15,
    "motivation": 0.1,
...
def predict_volleyball_match(team_a, team_b, match_date):
    api_home = search_volleyball_team(team_a)
    api_away = search_volleyball_team(team_b)

    if not api_home or not api_away:
        return {"error": f"Team not found: {team_a} vs {team_b}"}

    h2h_matches = get_last_six_volleyball_matches(api_home, api_away)
    if not h2h_matches:
        return default_volleyball_prediction(api_home, api_away, match_date)

    patterns = detect_volleyball_pattern(h2h_matches)
