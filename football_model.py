...
def predict_football_match(team_a, team_b, match_date):
    h2h_matches = get_last_six_football_matches(team_a, team_b)
    if not h2h_matches:
        return default_football_prediction(team_a, team_b, match_date)
...
def get_team_stats(team_name):
    # Simulated stats lookup – replace with API later
    return {
        "key_players": 8.0,
        "bench_depth": 7.0,
